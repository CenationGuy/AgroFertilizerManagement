...
<div class="container">
<h1 style="margin-top: 60px;">Inhibitors</h1>
<div id="message" ></div>
...
?>
  <?php echo $message; ?>
      <?php
      $stmt = $link->prepare('SELECT p_id,p_name,qty,price,p_image,description FROM products,categories
      WHERE cat_id = cat_num and cat_id = 8');
      $stmt->execute();
      ...
      while ($rows = $result->fetch_assoc()):
              ...
              $p_remain="";
              if($rows['qty']==0){
                $p_remain = "Item Unavailable ";
              } else if($rows['qty']==1){
                $p_remain = "Only one item left, Hurry Up!! ";
              }else{
                $p_remain="Available";
              }
              ?>
        ...
        <div class="card-deck">
          <div class="card p-2 border-secondary mb-2">
            <img src="<?= $rows['p_image'] ?>" class="card-img-top" height="150" width="150px">
            ...
            <div class="card-footer p-1">
              <form action="" class="form-submit">
                <div class="row p-2">
                  ...
                  <div class="col-md-6">
                    <input type="number"  min=1 oninput="validity.valid||(value='');"class="form-control pqty" value="<?= $quantity ?>" style="width:55px;">
                  </div>
                </div>
                ...
                <p class="rem"><?php echo $p_remain; ?></p>
                <?php
                if($rows['qty']==0){
                  ?><button class="btn btn-info btn-block addItemBtn" disabled style="margin-bottom: 30px;"><i class="fas fa-cart-plus"></i>&nbsp;&nbsp;Add to cart</button>
                <?php } ?>
                <?php if($rows['qty']>0) { ?>
